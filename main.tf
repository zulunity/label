locals {
  ...
  cloud_provider  = lower(replace(coalesce(local.input.cloud_provider, local.sentinel), local.regex_replace_chars, local.replacement))
  account         = lower(replace(coalesce(local.input.account, local.sentinel), local.regex_replace_chars, local.replacement))
  ...
  domain          = lower(replace(coalesce(local.input.domain, local.sentinel), local.regex_replace_chars, local.replacement))
